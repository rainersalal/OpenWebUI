...
version: 1.3
license: MIT
description: A filter pipeline for retrieving and querying documents from a Google Workspace Shared Drive
...
class Pipeline:
    ...
    def __init__(self):
        self.type = "filter"
        self.name = "Google Shared Drive RAG"
    ...
    async def on_startup(self):
        ...
        print("🚀 Starting Google Drive RAG Filter...")
        
        if not self.valves.GOOGLE_DRIVE_CREDENTIALS:
            print("⚠️  No Google Drive credentials provided.")
            print("💡 Configure credentials in Admin Panel > Settings > Pipelines")
            return
        
        if not self.valves.SHARED_DRIVE_ID:
            print("⚠️  No Shared Drive ID provided.")
            print("💡 Configure Shared Drive ID in Admin Panel > Settings > Pipelines")
            return
